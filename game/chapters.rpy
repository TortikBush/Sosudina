...
init python:
    def complete_chapter(chapter_id):
        if chapter_id in chapter_progress:
            chapter_progress[chapter_id]["completed"] = True
            chapter_num = int(chapter_id.split("_")[1])
            next_chapter = f"chapter_{chapter_num + 1}"
            if next_chapter in chapter_progress:
                chapter_progress[next_chapter]["unlocked"] = True